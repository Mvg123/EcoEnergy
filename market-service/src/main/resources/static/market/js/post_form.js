...
function prepareFormData() {
    ...
    const msg = document.getElementById('contractMsg'); // 추가한 메시지 박스

    const targetElement = document.querySelector('input[name="contractTypes"]');

    // 1. 하나도 체크 안 했으면?
    if (checkboxes.length === 0) {
        if(msg) msg.style.display = 'block'; // 빨간 글씨 보이기

        if (targetElement) {
            // 화면 중앙으로 부드럽게 스크롤 이동
            targetElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
            // 키보드 사용자 등을 위해 첫 번째 체크박스에 포커스
            targetElement.focus();
        }
    ...
    // 2. 체크 했으면?
    if(msg) msg.style.display = 'none'; // 빨간 글씨 숨기기
